...
# Initial params
# =============================================================================
sim = 'Hydrangea'# 'Magneticum'  'TNG_100'  'Horizon_AGN'
cluster = '116'
orientation = 'xy'
iteration = 1
...
patchs = Patchs(im,1850)  # Change size as necessary -- or equal to image size if the image is small

for patch in patchs:
    im[patch] = CircularHandMask(im[patch], mask_value=np.nan, vmin=0., vmax=5000) # Change vmin & max as necessary 
    im[patch] = CircularHandMask(im[patch], mask_value=np.nan, vmin=0., vmax=100) # Change vmin & max as necessary 

#im[patchs[3]] = CircularHandMask(im[patchs[3]], mask_value=np.nan, vmin=0., vmax=100) # Change vmin & max as necessary 
#im[patchs[4]] = CircularHandMask(im[patchs[4]], mask_value=np.nan, vmin=0., vmax=100) # Change vmin & max as necessary 
