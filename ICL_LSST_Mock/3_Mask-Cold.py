...
sim =  'Hydrangea' # 'Horizon_AGN'
clusters = ['294', '213', '294']
...
for cluster in clusters:    
    for orientation in orientations:
        ...
        for patch in patchs:
            im_chunks.append(im[patch])
            
            
        
        def patch_mask(im_chunk):
        
            threshold = detect_threshold(im_chunk, nsigma=1.2, sigclip_sigma=2, mask_value = np.nan)
            mask_chunk = np.zeros_like(im_chunk)
            try:
                # Segmentation - detecting sources
                segm = detect_sources(im_chunk, threshold, npixels=700,filter_kernel=kernel)
                # Deblending sources 
                mask_chunk = deblend_sources(im_chunk, segm, npixels=700, filter_kernel=kernel).data
            except:
        ...
        #mask[bcg_patch] = DeleteMaskRegion(mask[bcg_patch],im[bcg_patch], vmax=400)
        
        for ind, patch in enumerate(patchs):
            mask[patch] = DeleteMaskRegion(mask[patch],im[patch], vmax=400)
   
        # In case I want to remove more masks in other patchs [patchs index are from 0 to 9]
        #mask[patchs[1]] = DeleteMaskRegion(mask[patchs[1]],im[patchs[1]], vmax=400)
        #mask[patchs[6]] = DeleteMaskRegion(mask[patchs[6]],im[patchs[6]], vmax=400)
        ...
        # Save masked image
        # =============================================================================
        save_fits_image(im, hdr, res_dir+'Masks/', 'cold_'+os.path.basename(im_files[0]))
