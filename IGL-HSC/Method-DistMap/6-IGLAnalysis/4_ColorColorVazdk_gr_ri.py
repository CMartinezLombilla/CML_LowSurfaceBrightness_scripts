...
gal_rlim = 50 # pix; Rmax up to where obtain the colors of the galaxies
IGL_lowrlim = 90 # pix
IGL_uprlim = 250 # pix
...
files = {'g-i':[results_path+'IGL/color/profiles/dist_IGL_prof_g-i.fits', results_path+'IGL/color/profiles/dist_group_prof_g-i.fits'],
         'g-r':[results_path+'IGL/color/profiles/dist_IGL_prof_g-r.fits', results_path+'IGL/color/profiles/dist_group_prof_g-r.fits'], 
         'r-i':[results_path+'IGL/color/profiles/dist_IGL_prof_r-i.fits', results_path+'IGL/color/profiles/dist_group_prof_r-i.fits']}
...
# Mag, colors, M/L in SDSS filters (AB system);
# Set of solar-scaled theoretical isocrones from Padova+00 (Girardi+2000)
# IMF shape from Chabrier
# Assume metallicity as [M/H]=[Fe/H] because these can be cosidered as 'base' models
...
r = Table.read(g_i_IGL)['r'] # La posicion radial es la misma para todos los perfiles
color_g_i = Table.read(g_i_IGL)['color']
err_g_i = Table.read(g_i_IGL)['err']


g_i_IGLGal = files['g-i'][1]

r_gal = Table.read(g_i_IGLGal)['r'] # La posicion radial es la misma para todos los perfiles
color_g_i_gal = Table.read(g_i_IGLGal)['color']
err_g_i_gal = Table.read(g_i_IGLGal)['err']
...
# Metallicity tracks
#plt.plot(g_r_SDSS_23[ages_in[0]:ages_in[-1]+1],r_i_SDSS_23[ages_in[0]:ages_in[-1]+1], lw=3, color=colors[0],  label='-2.3 [M/H]', zorder=1)
#plt.plot(g_r_SDSS_17[ages_in[0]:ages_in[-1]+1],r_i_SDSS_17[ages_in[0]:ages_in[-1]+1], lw=3, color=colors[1], label='-1.7', zorder=1)
ax.plot(g_r_SDSS_13[ages_in[0]:ages_in[-1]+1],r_i_SDSS_13[ages_in[0]:ages_in[-1]+1], lw=3, color=colors[2], label='-1.3 [M/H]', zorder=1)
ax.plot(g_r_SDSS_07[ages_in[0]:ages_in[-1]+1],r_i_SDSS_07[ages_in[0]:ages_in[-1]+1], lw=3, color=colors[3], label='-0.7', zorder=1)
ax.plot(g_r_SDSS_04[ages_in[0]:ages_in[-1]+1],r_i_SDSS_04[ages_in[0]:ages_in[-1]+1], lw=3, color=colors[4], label='-0.4', zorder=1)
ax.plot(g_r_SDSS_00[ages_in[0]:ages_in[-1]+1],r_i_SDSS_00[ages_in[0]:ages_in[-1]+1], lw=3, color=colors[5], label='0.0', zorder=1)
ax.plot(g_r_SDSS_02[ages_in[0]:ages_in[-1]+1],r_i_SDSS_02[ages_in[0]:ages_in[-1]+1], lw=3, color=colors[6], label='0.2', zorder=1)
...
for ind, ind_table in enumerate(ages_in):
    ...
    ax.plot(np.arange(ind_table)*-1,np.arange(ind_table)*-1,'o', markersize=ages_ms[ind], markeredgecolor='black', linewidth=1, markerfacecolor='white', alpha=0.6, zorder=2, label=ages_label[ind])
    ax.plot(g_r_SDSS_13[ind_table],r_i_SDSS_13[ind_table],'o', markersize=ages_ms[ind], color=colors[2], alpha=0.6, zorder=2)
    ax.plot(g_r_SDSS_07[ind_table],r_i_SDSS_07[ind_table],'o', markersize=ages_ms[ind], color=colors[3], alpha=0.6, zorder=2)
    ax.plot(g_r_SDSS_04[ind_table],r_i_SDSS_04[ind_table],'o', markersize=ages_ms[ind], color=colors[4], alpha=0.6, zorder=2)
    ax.plot(g_r_SDSS_00[ind_table],r_i_SDSS_00[ind_table],'o', markersize=ages_ms[ind], color=colors[5], alpha=0.6, zorder=2)
    ax.plot(g_r_SDSS_02[ind_table],r_i_SDSS_02[ind_table],'o', markersize=ages_ms[ind], color=colors[6], alpha=0.6, zorder=2)

ax.legend(loc=4, fontsize=14, numpoints=1, ncol=2, framealpha=1,markerscale=0.87)	
ax.set_ylabel("$r-i$")
ax.set_xlabel("$g-r$")
ax.set_ylim(0.152, 0.38)
...
mean_IGLerr_g_i = np.nanstd(err_g_i[IGL_region])

mean_GALerr_g_r = np.nanstd(err_g_r_gal[gal_region])
mean_GALerr_r_i = np.nanstd(err_r_i_gal[gal_region])
mean_GALerr_g_i = np.nanstd(err_g_i_gal[gal_region])

ax.errorbar(mean_IGLcolor_g_r, mean_IGLcolor_r_i, xerr=mean_IGLerr_g_r, yerr=mean_IGLerr_r_i, marker='^', markersize=14, linestyle='None', color='forestgreen', zorder=3, alpha = 0.9,)
ax.errorbar(mean_GALcolor_g_r, mean_GALcolor_r_i, xerr=mean_GALerr_g_r, yerr=mean_GALerr_r_i, marker='s',linestyle='None', markersize=14, color='k', zorder=3, alpha = 0.9)
...
ax_trampa = ax.twinx()
ax_trampa.errorbar(0,0,0,0, marker='^', markersize=14, linestyle='None', color='forestgreen', alpha = 0.9, label='IGL')
ax_trampa.errorbar(0,0,0,0, marker='s',linestyle='None', markersize=14, color='k',  alpha = 0.9, label='Group')
ax_trampa.legend(loc=2)


ax_trampa.set_ylim(1,2)
ax_trampa.set_ylim(0.152, 0.38)
ax_trampa.set_yticklabels([])
ax_trampa.yaxis.get_ticklocs(minor=True)
ax_trampa.minorticks_on()
ax_trampa.tick_params(which='both', direction="in")
...
plt.savefig(results_path+'IGL/StellPop/dist_Color_Color.pdf')


paper_path = '/Users/z3530031/Dropbox/Aplicaciones/Overleaf/IGL-MNRAS/figures/'
plt.savefig(paper_path+'Color_Color.pdf')
