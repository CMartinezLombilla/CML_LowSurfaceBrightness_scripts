...
for ind, ind_table in enumerate(ages_in):
    ...
    ax.plot(g_r_SDSS_13[ind_table],r_i_SDSS_13[ind_table],'o', markersize=ages_ms[ind], color=colors[2], alpha=0.6, zorder=2, label=ages_label[ind])
...
ax.legend(loc=4, fontsize=14, numpoints=1, ncol=2, framealpha=1)	
ax.set_ylabel("$r-i$")
ax.set_xlabel("$g-r$")
...
ax2.errorbar(mean_IGLcolor_g_r, mean_IGLcolor_r_i, xerr=mean_IGLerr_g_r, yerr=mean_IGLerr_r_i, marker='^', markersize=14, linestyle='None', color='forestgreen', zorder=3, alpha = 0.9, label='IGL')
ax2.errorbar(mean_GALcolor_g_r, mean_GALcolor_r_i, xerr=mean_GALerr_g_r, yerr=mean_GALerr_r_i, marker='s',linestyle='None', markersize=14, color='k', zorder=3, alpha = 0.9, label='Group')
ax2.legend(loc=2)
