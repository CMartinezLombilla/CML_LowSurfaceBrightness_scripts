...
IGL_2Dmod_Lum = {'Name':'IGL_2Dmod_Lum [Lsun]', 'g':0, 'r':0, 'i':0}
...
IGLfrac_SB_Lum = {'Name':'IGL_SB_Lum [Lsun]', 'g':0, 'r':0, 'i':0}
...
pdf = PdfPages(results_path+'IGL/IGLfraction/dist_All_appertures.pdf')
...
for ind, band in enumerate(bands):
    ...
    AbsMag = magnitudes(TotalFlux[band], zp, z=z, k_corr=k_corrs[band], band=band)['M_'+band]
    TotalLum[band] = luminosity(AbsMag, band=band)
...
# pdf file to save the plots
pdf = PdfPages(results_path+'IGL/IGLfraction/dist_IGL-2DMod_ap'+str(maxsma['Core'])+'pix.pdf')
...
for ind, band in enumerate(bands):
    ...
    IGLfrac_2DMod_err[band] = np.sqrt(np.square(IGL_2Dmod_sum_err[band] / IGL_2Dmod_sum[band]) 
                                                            + np.square(TotalFlux_err[band] / TotalFlux[band]))
    
    AbsMag = magnitudes(IGL_2Dmod_sum[band], zp, z=z, k_corr=k_corrs[band], band=band)['M_'+band]
    IGL_2Dmod_Lum[band] = luminosity(AbsMag, band=band)  
...
sb_IGLlim_list = [26, 26.5, 27, 27.5, 28]
...
for band in bands:
    ...
    data_IGL = fits.getdata(results_path+'masks/DeconvGroupMasked-'+band+'.fits')
    #data_IGL = fits.getdata(results_path+'masks/GroupMasked-'+band+'.fits')
    ...
    # =============================================================================
    # METHOD : convert counts in the image to sb values accounting for SB dimming 
    # =============================================================================
    IGL_sum_SBi = []
    IGL_sum_err_SBi = []
    IGLfrac_SBi = []
    ...
    data_sb = c2sb.do(data_IGL)
    
    # Masking below image sb_lim (HSC_sb_lim)
    data_down = np.copy(data_sb)
    data_down[data_sb>HSC_sb_lim[band]] = np.nan
    ...
    for ind, sblim in enumerate(sb_IGLlim_list): # correcting by sbdimming & k-corr
        ...
        data_sb_IGL_ii = np.copy(data_down)
        data_sb_IGL_ii[data_sb<sblim] = np.nan
        
        data_IGL_ii = sb2counts(data_sb_IGL_ii, zp, pixscale, A=0, dimming=0, k_corr=0)
        
        # =============================================================================
        #  Get appertures and sum all pixels inside the apperture of the core region          
        # =============================================================================
        IGL_core, IGL_core_err, IGL_core_ap = flux_in_ellipse(data_IGL_ii, 
                                             Xc_core, 
                                             Yc_core, 
                                             maxsma['Core'], 
                                             ell['Core'], 
        ...
        # Get appertures and sum all pixels inside the ellipse 
        Mem545_sum, Mem545_sum_err, Mem545_ap = flux_in_ellipse(data_IGL_ii, 
                                                            Xcoords[mem_in], 
                                                            Ycoords[mem_in], 
                                                            maxsma['1660545'], 
                                                            ell['1660545'], 
        ...
        # Get appertures and sum all pixels inside the ellipse 
        Mem069_sum, Mem069_sum_err, Mem069_ap = flux_in_ellipse(data_IGL_ii, 
                                                            Xcoords[mem_in], 
                                                            Ycoords[mem_in], 
                                                            maxsma['2301069'], 
                                                            ell['2301069'], 
        ...
        IGL_sumii = IGL_core + Mem545_sum + Mem069_sum
        IGL_sum_SBi.append(IGL_sumii)

        IGL_errii = np.sqrt(np.square(IGL_core_err) + np.square(Mem545_sum_err) + np.square(Mem069_sum_err))            
        IGL_sum_err_SBi.append(IGL_errii)
        
        AbsMag = magnitudes(IGL_sumii, zp, z=z, k_corr=k_corrs[band], band=band)['M_'+band]
        IGL_sb_lum_ii.append(luminosity(AbsMag, band=band))
        ...
        save_fits_image(data_IGL_ii, 
                        ...
                        'dist_IGLfrac-SBlim-'+str(sblim)+'-'+band+'.fits')
    ...
    IGLfrac_SB_Lum[band] = IGL_sb_lum_ii
...
for ii, sblim in enumerate(sb_IGLlim_list):
    ...
    lum_sb_err = {'Name':'IGL_SB_Lum [Lsun] '+str(sblim),'g':IGLfrac_SB_Lum['g'][ii], 'r':IGLfrac_SB_Lum['r'][ii], 'i':IGLfrac_SB_Lum['i'][ii]}
    table_vec.append(lum_sb_err)
...
table_IGL.write(results_path+'IGL/IGLfraction/dist_IGLfrac_table.fits', overwrite=True)
